...
const UpdateInventory = () => {
  ...
  const getOneInventory = useInventoryStore((state) => state.getOneItem);
  const updateInventory = useInventoryStore((state) => state.updateItem);
  ...
  useEffect(() => {
    if(typeof id?.id === 'string') {
      const itemData = getOneInventory(id?.id);

      if (itemData) {
        setValue('id', itemData.id);
        setValue('code', itemData.code);
        setValue('name', itemData.name);
        setValue('stock', itemData.stock);
  ...
  const onSubmitForm = () => {
    ...
    updateInventory(dataForm?.id, dataForm);
    toast.success('Sukses edit produk');
    reset();
    ...
    <div className="h-full flex flex-col p-0 md:p-3 xl:p-6">
      <div className='h-full'>
        <form onSubmit={handleSubmit(onSubmitForm)} className="flex flex-col gap-10 h-full items-center">
          ...
          {/* Form Input */}
          <div className="space-y-6 w-full lg:max-w-[80%]">
                ...
                <div>
                  ...
                  <label htmlFor="file-upload">
                    <div className="w-full min-h-52 bg-(--primary-color) rounded-lg flex justify-center items-center flex-col gap-2 border-2 border-dashed hover:border-(--text-primary-color)/20 hover:bg-(--primary-color)/40 cursor-pointer border-[#101625] duration-300 group/image-cover">
                      {imagePreview ? (
                        <Image alt='Preview Image' width={50} height={50} src={imagePreview} className="w-full h-52 rounded-lg object-center" />
                      ) : (
                        <>
                          <Upload className="text-(--text-primary-color)/20" size={50} />
                          <span className="text-(--text-primary-color)/70 text-[0.875rem] group-hover/image-cover:text-(--text-primary-color)/40 duration-300">
          ...
          {/* Button submit */}
          <div className="w-full lg:max-w-[80%] m-auto flex justify-center">
            <button
              type="submit"
              className={cn("duration-300 min-h-[40px] bg-(--orange-color) hover:bg-(--orange-color)/80 w-full rounded-lg text-[0.813rem] font-bold text-(--text-primary-color) cursor-pointer")}
            >
              Simpan
            </button>
          </div>
